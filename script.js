...
const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwKym3EhjYMp2aS6cRoVv5TEXErEHuPCPtwGeHlp_AVlVnbx4zWwciFVYcQ0d8Oo7E6Mw/exec';
...
// ========== ОТПРАВКА ДАННЫХ БЕЗ CORS (mode: 'no-cors') ==========
async function submitToGoogleSheets(formData) {
  ...
  try {
    await fetch(GOOGLE_SCRIPT_URL, {
      method: 'POST',
      mode: 'no-cors',               // <- главный флаг, отключает preflight
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
      },
      body: urlEncodedData
    });
    // При mode: 'no-cors' мы не можем получить ответ, но запрос отправлен.
    // Поэтому всегда возвращаем true (браузер не выдаст ошибку CORS)
    return true;
  } catch (error) {
...
function initRSVPForm() {
  ...
  form.addEventListener('submit', async function (e) {
    ...
    // Резервное копирование в localStorage (на случай, если интернет пропадёт)
    try {
    ...
    if (success) {
      ...
      alert('Произошла ошибка. Данные сохранены локально, мы свяжемся с вами.');
    }
  });
}
